 ...
 */
NTSTATUS SMBios::Spoof()
{
	auto* base = Utils::GetModuleBase(XOR("ntoskrnl.exe"));
	if (!base)
	{
		ERROR(XOR("Failed to find ntoskrnl.sys base!\n"));
		return STATUS_UNSUCCESSFUL;
	}

	auto* physicalAddress = static_cast<PPHYSICAL_ADDRESS>(Utils::FindPatternImage(base, XOR("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x74\x00\x8B\x15"), XOR("xxx????xxxx?xx"))); // WmipFindSMBiosStructure -> WmipSMBiosTablePhysicalAddress
	if (!physicalAddress)
	{
		ERROR(XOR("Failed to find SMBIOS physical address!\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	if (!physicalAddress)
	{
		ERROR(XOR("Physical address is null!\n"));
		return STATUS_UNSUCCESSFUL;
	}

	auto* sizeScan = Utils::FindPatternImage(base, XOR("\x8B\x1D\x00\x00\x00\x00\x48\x8B\xD0\x44\x8B\xC3\x48\x8B\xCD\xE8\x00\x00\x00\x00\x8B\xD3\x48\x8B"), XOR("xx????xxxxxxxxxx????xxxx"));  // WmipFindSMBiosStructure -> WmipSMBiosTableLength
	if (!sizeScan)
	{
		ERROR(XOR("Failed to find SMBIOS size!\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	if (!mapped)
	{
		ERROR(XOR("Failed to map SMBIOS structures!\n"));
		return STATUS_UNSUCCESSFUL;
	}
