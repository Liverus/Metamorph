...
NTSTATUS Disks::DiskLoop(PDEVICE_OBJECT deviceArray, RaidUnitRegisterInterfaces registerInterfaces)
{
	...
	while (deviceArray->NextDevice)
	{
		if (deviceArray->DeviceType == FILE_DEVICE_DISK)
		{
			...
			LOG(XOR("Changed disk serial %s to %s.\n"), original, buffer);
 ...
 */
NTSTATUS Disks::Spoof()
{
	auto* base = Utils::GetModuleBase(XOR("storport.sys"));
	if (!base)
	{
		LOG(XOR("Failed to find storport.sys base!\n"));
		return STATUS_UNSUCCESSFUL;
	}

	const auto registerInterfaces = static_cast<RaidUnitRegisterInterfaces>(Utils::FindPatternImage(base, XOR("\x48\x89\x5C\x24\x00\x55\x56\x57\x48\x83\xEC\x50"), XOR("xxxx?xxxxxxx"))); // RaidUnitRegisterInterfaces
	if (!registerInterfaces)
	{
		LOG(XOR("Failed to find RaidUnitRegisterInterfaces!\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	for (auto i = 0; i < 2; i++)
	{
		const wchar_t* raidFormat = XORW(L"\\Device\\RaidPort%d");
...
{
	auto* base = Utils::GetModuleBase(XOR("disk.sys"));
	if (!base)
	{
		LOG(XOR("Failed to find disk.sys base!\n"));
		return STATUS_UNSUCCESSFUL;
	}

	const auto disableFailurePrediction = static_cast<DiskEnableDisableFailurePrediction>(Utils::FindPatternImage(base, XOR("\x48\x89\x5C\x24\x00\x48\x89\x7C\x24\x00\x55\x48\x8D\x6C\x24\x00\x48\x81"), XOR("xxxx?xxxx?xxxxx?xx"))); // DiskEnableDisableFailurePrediction
	...
	RtlInitUnicodeString(&driverDisk, XORW(L"\\Driver\\Disk"));
	...
	if (!NT_SUCCESS(status))
	{
		LOG(XOR("Failed to get disk driver object!\n"));
		return STATUS_UNSUCCESSFUL;
	}
	...
	if (!NT_SUCCESS(status))
	{
		LOG(XOR("Failed to enumerate disk driver device object list!\n"));
		return STATUS_UNSUCCESSFUL;
	}
